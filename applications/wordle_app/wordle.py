...
def on_key_press(char):
    ...
    with use_scope(
        f"s-{session_local.curr_row}-{len(session_local.curr_word)}", clear=True
    ):
        put_text(char)
        ...
        if not is_word(session_local.curr_word):
            ...
            for idx, c in enumerate(session_local.curr_word):
                ...
                if TODAY_WORD[idx] == c:
                    ...
                    run_js(
                        '$("button:contains(%s)").css({"background-color":"#6aaa64", "color":"white"})'
                        % c
                    )
                    text_bg = "#6aaa64"
                    ...
                    if c not in session_local.green_chars:
                        run_js(
                            '$("button:contains(%s)").css({"background-color":"#c9b458", "color":"white"})'
                            % c
                        )
                else:
                    text_bg = "#787c7e"
                    session_local.game_result += "⬜"
                    run_js(
                        '$("button:contains(%s)").css({"background-color":"#787c7e", "color":"white"})'
                        % c
                    )
        ...
        if session_local.game_pass:
            ...
            with popup("Game Result", size="small"):
                ...
                put_button(
                    "Share",
                    color="success",
                    onclick=lambda: toast("Copied to clipboard")
                    or run_js(
...
@config(
    ...
    description="A PyWebIO implementation",
    css_style=CSS,
)
def main():
    put_markdown(
        "# WORDLE \n A Pure PyWebIO implementation of Wordle game, by using PyWebIO library. "
        "[Source code](https://github.com/pywebio/wordle/blob/master/wordle.py)"
    ).style("text-align:center")
    ...
    keyboard = [
        put_buttons(
            [dict(label=c, value=c, color="light") for c in keys],
            on_key_press,
            serial_mode=True,
        )
